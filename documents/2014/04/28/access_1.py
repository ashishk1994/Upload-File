...
_MESSAGE_NO_ORG_APP_SURVEY_DEFINED = translation.ugettext(
    'You first need to fill the organization application form.')	
...
class IsOrgAppSurveyDefinedAccessChecker(AccessChecker):
  """AccessChecker that ensures that the user has filled Organization application survey."""

  def checkAccess(self, data, check):
    """See AccessChecker.checkAccess for specification."""
    if not data.org_app:
      raise exception.Forbidden(message=_MESSAGE_NO_ORG_APP_SURVEY_DEFINED)

IS_ORG_APP_SURVEY_DEFINED_ACCESS_CHECKER = IsOrgAppSurveyDefinedAccessChecker()
